...
import './App.css';
import { Typography, Divider, List, ListItem, CircularProgress, LinearProgress, Grid, Container, Button, TextField } from '@material-ui/core';
...
const Standings = ({ inViewport, forwardedRef, tournament, ifpaId, claimedBy, playerClicked, tournamentInfo, standingsInfo }) => {
      ...
      <div ref={forwardedRef}>
        ...
        <Grid container wrap="nowrap" spacing={5}>
          ...
          {games && standing && <Grid item xs={3} justify="left">
              ...
              {wonBy && <Typography onClick={() => playerClicked(wonBy)} className={wonBy.claimed_by ? classes.underline : null}>{`Lead: ${wonBy.name}`}</Typography>}
  ...
  const Tournament = ({tournament, tournamentId, ifpaId, claimedBy, playerClicked}) => {
    ...
    return tournamentId ? <div>{tournamentInfo && <ListItem><div style={{width: '100%', display: 'flex', alignItems: 'center'}}><Close onClick={unpin} className={classes.underline} style={{color: 'red', fontSize: '20px', marginTop: '-2px', marginRight: '2px'}} /><a target="_blank" href={`https://matchplay.events/live/${tournamentInfo.url_label}`}><House style={{paddingTop: '1px', marginRight: '5px'}} /></a><div style={{width: '100%'}}>{` ${tournamentInfo.name} (${tournamentInfo.type})`}</div></div>
      ...
      </Grid><Divider/><div style={{margin: '15px 0', display: 'flex', color: 'blue'}}>Selected Player Results:</div></div>}
...
function App() {
  ...
  function fetchPlayer(claimedBy) {
    const id = claimedBy || matchPlayId;
    localStorage.setItem('last:fetched:player', id);
    setPlayer(null);
    setTournaments(null);
    setMessage(null);
    isProgress(true);
    fetch(`https://matchplay.events/data/users/${id}`)
      .then(response => {
        if (!response.ok) {
          setMessage(`Player with the id ${id}: ${response.status}`);
        }
        return response.json();
      })
      .then(json => {
        ...
        isProgress(false);
        setMessage(`Player with the id:${id} not found: ${e}`);
      })
    return;  
  };
  ...
  function fetchTournaments() {
    ...
    fetch(`https://matchplay.events/data/users/${matchPlayId}/played`)
      .then(response => {
        if (!response.ok) {
          setMessage(`Tournaments for the player id:${matchPlayId}: ${response.status}`);
        }
        return response.json();
      })
      .then(json => {
        ...
        isProgress(false);
        setMessage(`Tournaments for the player id:${matchPlayId} not found: ${e}`);
      })
    return;  
  };
  
  /*function fetchIfpa() {
    fetch(`https://api.ifpapinball.com/v1/rankings?api_key=ac0a98451d70551bbcd0a7ddce7acb6b`, {
      credentials: 'include'
    })
      .then(response => {
        return response.json();
  ...
  function onPlayerClicked(player) {
    ...
    if (claimedBy) {
        setMatchPlayId(claimedBy);
        fetchPlayer(claimedBy);
        return;
    }
  ...
  return (
    <div className="App">
        <div>
          <Grid justify="center" style={{margin:'20px 10px'}} container spacing={1} alignItems="flex-end">
            <Grid item>
              <AccountCircle />
            </Grid>
            <Grid item>
              <TextField label="MatchPlay-Id" onChange={onChangeTextFieldMatchPlayId} value={matchPlayId} />
            </Grid>
            <Grid item>
              <Button onClick={() => fetchPlayer()}variant="outlined" color="primary">Search</Button>    
            </Grid>
            {(tournament || matchPlayId) && <Grid item>
              <Button onClick={() => reset()}variant="outlined" color="primary">Reset</Button>    
            </Grid>}
          </Grid>
        </div>
        {tournament ? <Container>
          {player && <Tournament tournamentId={tournament} ifpaId={player.ifpa_id} claimedBy={Number(matchPlayId)} playerClicked={(p) => onPlayerClicked(p)}/>}
          {progress && <div><CircularProgress style={{margin: '5px 0 5px 0'}}/></div>}
        </Container> : 
        <Container>
          {message && <div style={{margin:'5px 0 5px 0'}}>{message}</div>}
          {player && <a target="_blank" href={`https://www.ifpapinball.com/player.php?p=${player.ifpa_id}`}>{`${player.first_name} ${player.last_name} #ifpa: ${player.ifpa_id}`}</a>}
          {player && tournaments && <Divider style={{margin: '15px 0 5px 0'}}/>}
          {tournaments && <div><div style={{margin: '10px 0 10px 0'}}>Matchplay Tournaments</div><Divider style={{margin: '5px 0 5px 0'}}/><List>{
            tournaments.map((t) => {
              return <Tournament tournament={t} ifpaId={player.ifpa_id} claimedBy={Number(matchPlayId)} playerClicked={(p) => onPlayerClicked(p)}/>;
            })
          }</List></div>}
          {progress && <div><CircularProgress style={{margin: '5px 0 5px 0'}}/></div>}
        </Container>}
